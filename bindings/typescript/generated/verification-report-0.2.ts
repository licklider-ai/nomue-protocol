...
export type ExecutionOutcome2A_ConformanceResult = {
  [k: string]: unknown;
} & {
  check_id: string;
  execution: "completed" | "not_run" | "error";
  outcome?: "pass" | "fail" | "indeterminate";
  scope: ExecutionOutcome2A_Scope;
  reason_codes: ExecutionOutcome2A_ReasonCode[];
  error?: ExecutionOutcome2A_ErrorObject;
};
export type ExecutionOutcome2A_ReasonCode = string;
export type ExecutionOutcome2A_CheckResult = {
  [k: string]: unknown;
} & {
  check_id: string;
  ...
  scope: ExecutionOutcome2A_Scope;
  reason_codes: ExecutionOutcome2A_ReasonCode[];
  evidence?: ExecutionOutcome2A_Evidence;
  error?: ExecutionOutcome2A_ErrorObject;
};
...
/**
 * The Phase 2A verification report. A separate artifact from the Record (NRS-CORE-0008), with no overall status, no VERIFIED value, no significance boolean, and no whole-record validity boolean; every result is inseparable from its check identifier, version, and scope. The guarantee boundary makes the non-claims explicit: scientific validity, declaration truth, distributional-model validity, causal interpretation, and standardized effect size are all not_asserted.
 */
export interface Phase2ANomueVerificationReport {
  $schema: "urn:nomue:schema:verification-report:0.2.0-draft.1";
  report_type: "nomue-verification-report";
  record_reference: ReportSchema2A_RecordReference;
  interpretation_bundle_id: CommonIdentifier_Uri;
  verifier: ReportSchema2A_Verifier;
  generated_at: string;
  conformance: ExecutionOutcome2A_ConformanceResult;
  verification_results: ExecutionOutcome2A_CheckResult[];
  guarantee_boundary: ReportSchema2A_GuaranteeBoundary;
}
export interface ReportSchema2A_RecordReference {
  record_id: CommonIdentifier_Uri;
  revision_id: CommonIdentifier_Uri;
  content_digest: string;
}
export interface ReportSchema2A_Verifier {
  name: string;
  version: string;
  source_commit: string;
}
export interface ExecutionOutcome2A_Scope {
  kind: "record" | "record_revision" | "analysis" | "result";
  id: CommonIdentifier_Uri;
}
export interface ExecutionOutcome2A_ErrorObject {
  error_type: string;
  message: string;
}
export interface ExecutionOutcome2A_Evidence {
  ...
  recomputed?: ExecutionOutcome2A_RecomputedGuaranteeResult;
  mismatches?: ExecutionOutcome2A_Mismatch[];
  /**
   * @minItems 1
   */
  unsupported_declarations?: [
    ExecutionOutcome2A_UnsupportedDeclaration,
    ...ExecutionOutcome2A_UnsupportedDeclaration[]
  ];
}
export interface ExecutionOutcome2A_RecomputedGuaranteeResult {
  /**
   * @minItems 2
   * @maxItems 2
   */
  group_summaries: [ExecutionOutcome2A_GroupSummary, ExecutionOutcome2A_GroupSummary];
  effect_estimate: {
    estimate: number;
    standard_error: number;
    confidence_interval: {
...
export interface ExecutionOutcome2A_GroupSummary {
  group_id: CommonIdentifier_LocalId;
  n: number;
  mean: number;
  sample_variance: number;
}
export interface ExecutionOutcome2A_Mismatch {
  quantity: string;
  group_id?: CommonIdentifier_LocalId;
  declared: number | string;
  recomputed: number | string;
}
export interface ExecutionOutcome2A_UnsupportedDeclaration {
  declaration: string;
  declared: string | number;
  supported: string;
}
export interface ReportSchema2A_GuaranteeBoundary {
