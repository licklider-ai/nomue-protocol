...
/**
 * Machine-readable refusal emitted when a normal verification report cannot be produced (NRS-CORE-0011). A refusal carries no overall status, no VERIFIED value, no verification outcome, and no scientific-validity evaluation; it is never a partial success (NRS-SEC-0004). Fields that could not be read from the input are omitted, never fabricated: there is no record reference, because no Record was interpreted. Supersedes urn:nomue:schema:verifier-refusal:0.2.0-draft.1 by adding the routing_error kind for bundle-independent routing failures (ADR-0017); the draft.1 schema is retained unmodified as a historical artifact.
 */
export type NomueVerifierRefusal = {
  [k: string]: unknown;
} & {
  $schema: "urn:nomue:schema:verifier-refusal:0.2.0-draft.2";
  ...
  limit_category?: "file_size" | "nesting_depth" | "observation_count" | "string_length" | "parser_exhaustion";
  verifier: {
    name: string;
    version: string;
  };
  ...
  generated_at: CommonIdentifier_Rfc3339UtcZ;
};
