...
/**
 * Machine-readable refusal emitted when a normal verification report cannot be produced (NRS-CORE-0011). A refusal carries no overall status, no VERIFIED value, no verification outcome, and no scientific-validity evaluation; it is never a partial success (NRS-SEC-0004). Fields that could not be read from the input are omitted, never fabricated: there is no record reference, because no Record was interpreted.
 */
export type NomueVerifierRefusal = {
  [k: string]: unknown;
} & {
  $schema: "urn:nomue:schema:verifier-refusal:0.2.0-draft.1";
  output_type: "nomue-verifier-refusal";
  refusal_kind: "parse_error" | "unsupported_bundle" | "resource_limit" | "canonicalization_failure" | "internal_error";
  /**
   * @minItems 1
   */
