...
C3's Unicode vector used an example containing a euro sign plus Japanese-script literal text. C7 uses `€ euro and Ω Unicode literal`. The vector's declared purpose is unchanged: non-ASCII characters remain literal while mandated control characters use the required escapes. The input hash, expected canonical bytes, and expected Record-content digest were regenerated for the substituted non-ASCII input.
