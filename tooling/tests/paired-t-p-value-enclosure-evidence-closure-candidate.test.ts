...
describe("paired-t p-value enclosure evidence closure candidate", () => {
  ...
  it("rejects artifact substitution, weakened closure evidence, and premature promotion", () => {
    ...
    overlap.closure_items.secondary_overlap_success_path.validator_requires_declared_and_exact_enclosure_overlap =
      false;
    expect(validatePairedTPValueEnclosureEvidenceClosureCandidate(overlap)).not.toEqual([]);

    const cell = mutableCheckpoint();
    cell.closure_items.exact_enclosure_endpoints_and_rounding_cells.strict_rounding_cell_containment =
      false;
  ...
  it("does not alter the current numerical-readiness incompleteness", () => {
    ...
    expect(readiness.numerical_contract_closure.closure).toBe("incomplete");
    expect(readiness.support_domain_closure.runtime_support_enabled).toBe(false);
  });
});
