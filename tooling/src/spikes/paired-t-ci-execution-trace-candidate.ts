...
function primitiveVerified(
  ...
  return (
    validatePairedTBinary64PrimitiveCandidate({ operation, operands: [...operands], result }).length === 0
  );
}
...
function expectedTraceFromG4(
  g4Trace: PairedTG4ExecutionTraceCandidate,
): { trace?: PairedTCIExecutionTraceCandidate; classification?: PairedTCIExecutionCandidateResult["classification"] } {
  ...
  if (critical === undefined) return { classification: "selected_critical_value_table_unavailable" };
...
export function verifyPairedTCIExecutionTraceCandidate(candidate: unknown): {
  ...
  const expectedKeys = ["format", "g4_trace", "node_count", "nodes", "outcome", "selected_table", "sha256"].sort();
...
export function evaluatePairedTCIExecutionTraceCandidate(candidate: unknown): PairedTCIExecutionCandidateResult {
  ...
  if (expected.trace === undefined) return refusal(expected.classification ?? "ci_trace_verification_failed");
