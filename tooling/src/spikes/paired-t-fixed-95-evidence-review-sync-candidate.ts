...
export function validatePairedTFixed95EvidenceReviewSyncCandidate(candidate: unknown): string[] {
  try {
    return JSON.stringify(canonicalizeJson(candidate)) ===
      ...
      ? []
      : ["fixed-95 evidence review-sync checkpoint differs from the reviewed M3 candidate"];
  } catch {
    return ["fixed-95 evidence review-sync checkpoint differs from the reviewed M3 candidate"];
  }
}
