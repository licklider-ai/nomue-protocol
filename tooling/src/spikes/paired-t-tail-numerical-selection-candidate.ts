/** Fail-closed validation for the non-authoritative R2-D5 tail numerical selection. */

export interface PairedTTailNumericalSelectionCandidate {
  status: string;
  issuance: string;
  review_issue: string;
  ...
  truth_error_contract: Record<string, unknown>;
  graph_and_table_binding: Record<string, unknown>;
  series_closure: Record<string, unknown>;
...
const EXPECTED_CHECKPOINT = {
  ...
  graph_and_table_binding: {
    runtime_graph: "tooling/src/spikes/paired-t-runtime-table-integration-candidate.ts",
    reviewed_inverse_beta_table_hash:
      "sha256:ba1f992199e9e153956589d62dcf5a6509575100bb7c923c170bfa45fdd76c08",
    table_integration_review:
      "governance/drafts/release-2-candidate/reviews/d5-runtime-table-integration-adversarial-review-disposition.md",
    supported_execution_surface: "tooling/src/spikes/paired-t-supported-execution-candidate.ts",
    supported_execution_review:
  ...
  series_closure: {
    ...
    remainder_kind: "a_posteriori_positive_geometric_upper_bound",
  ...
  projection_contract: {
    ...
    zero_subnormal_invalid_or_insufficient_margin: "fail_closed_refusal",
    projection_margin_runtime_activated: false,
  },
  failure_semantics: {
    table_trace_proof_or_projection_precondition_failure:
      "fail_closed_refusal_without_support_claim",
    final_public_reason_codes_frozen: false,
  ...
  prohibited_claims: [
    ...
    "supported_df_max",
    "selected_supported_platform_or_execution_predicate",
...
function canonicalizeJson(value: unknown, ancestors = new Set<object>()): unknown {
  if (
    ...
    throw new TypeError("tail numerical selection checkpoint contains non-JSON data");
  }
  if (typeof value !== "object" || value === null) return value;
  if (ancestors.has(value)) {
    throw new TypeError("tail numerical selection checkpoint contains a cycle");
  }
  const nextAncestors = new Set(ancestors).add(value);
  const keys = Reflect.ownKeys(value);
  if (keys.some((key) => typeof key === "symbol")) {
    throw new TypeError("tail numerical selection checkpoint contains symbol keys");
  }
  ...
  if (Array.isArray(value)) {
    ...
    if (
      ...
      throw new TypeError("tail numerical selection checkpoint contains an invalid array");
    }
    const result: unknown[] = [];
    for (let index = 0; index < lengthDescriptor.value; index += 1) {
      const descriptor = descriptors[String(index)];
      if (descriptor === undefined || !descriptor.enumerable || !("value" in descriptor)) {
        throw new TypeError("tail numerical selection checkpoint contains a non-JSON array entry");
      }
  ...
  if (Object.getPrototypeOf(value) !== Object.prototype) {
    throw new TypeError("tail numerical selection checkpoint contains a non-JSON object");
  }
  const entries: [string, unknown][] = [];
  for (const key of keys) {
    if (typeof key !== "string") {
      throw new TypeError("tail numerical selection checkpoint contains a non-string key");
    }
    const descriptor = descriptors[key];
    if (descriptor === undefined || !descriptor.enumerable || !("value" in descriptor)) {
      throw new TypeError("tail numerical selection checkpoint contains hidden or accessor data");
    }
...
export function validatePairedTTailNumericalSelectionCandidate(candidate: unknown): string[] {
  try {
