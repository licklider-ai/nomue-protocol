...
const EXPECTED_CHECKPOINT = {
  ...
  proof_model: {
    ...
    critical_value_truth:
      "exact_midpoint_rounding_cell_of_reviewed_correctly_rounded_binary64_cell",
...
function criticalValueTruth(criticalValueHex: string): {
...
function canonicalizeJson(value: unknown, ancestors = new Set<object>()): JsonValue {
  ...
  if (typeof value === "number") {
    if (!Number.isFinite(value))
      throw new TypeError("endpoint truth envelope contains non-finite data");
    return value;
  }
  if (
    value === undefined ||
...
export function verifyPairedTCIEndpointTruthCandidate(candidate: unknown): {
  ...
  const top = ownDataRecord(candidate, [
    "format",
    "ci_trace",
    "g4_truth_envelope",
    "proof",
