...
| R5 successor fixtures               | Positive projection/timing/report view plus missing mapping, source mismatch, missing timing, inadmissibility dependency and incomplete-evidence negatives |
...
`pnpm regression:phase1`, with their exact historical pins recorded. Unchanged
evidence comparison must also include `pnpm evidence:phase2a --check`,
`pnpm oracle:phase2a` and, when a candidate freeze exists,
`pnpm snapshot:manifest --check-candidate`. Existing zero-or-two selected-identity
failures remain conformance regression cases rather than new R5 semantics. No policy
registry or test-only policy fixture is introduced.
...
- Do not default absent projection inputs or timing status into a legacy Record.
