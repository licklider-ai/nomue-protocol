...
assert.ok(IDS.checks.arithmetic.startsWith("https://example.invalid/"));
...
fs.writeFileSync(new URL("./example-record.jcs", import.meta.url), bytes(baseline.record));
for (const [name, data] of [
  ...
  fs.writeFileSync(new URL("./" + name, import.meta.url), JSON.stringify(data, null, 2) + "\n");
console.log(
