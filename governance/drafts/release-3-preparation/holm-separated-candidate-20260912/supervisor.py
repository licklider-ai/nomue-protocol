...
def strict_transport(raw):
    ...
    if "verified_record_base64" in value:
        record = base64.b64decode(value["verified_record_base64"], validate=True)
        if len(record) > 2359296 or len(output.get("checks", [])) != 5:
            raise ValueError("forwarding shape")
        if any(c.get("execution") != "completed" or c.get("outcome") != "pass" for c in output["checks"]):
            raise ValueError("forwarding without all checks")
    return value
...
def run(args):
    ...
    flags, info = {}, {"kernel": platform.release(), "architecture": platform.machine()}
    ...
    try:
        ...
        env = {"PATH": "/usr/bin:/bin", "TMPDIR": str(temporary),
               "NOMUE_EXPERIMENT_PYTHON": args.python, "LANG": "C.UTF-8"}
        command = [args.node, str(HERE / "entry.mjs"), args.record, args.expected]
        # Test-only replacement entry is a trusted CLI argument, never a Record field.
        if args.probe:
            command = [args.node, str(HERE / "probes.mjs"), args.probe]
        process = subprocess.Popen([args.python, "-I", str(HERE / "supervisor.py"), "--bootstrap", str(leaf), *command],
    ...
    if not any(flags.values()):
        try:
            ...
            result = strict_transport(streams["stdout"])
        except Exception:
            flags["completed_invalid_output"] = True
    receipt = {"kind": "unissued-controlled-execution", "category": category(flags),
...
def main():
    ...
    p.add_argument("--probe", choices=["checkpoint-timeout", "node-memory", "worker-memory", "pids", "descendant", "stdout", "stderr", "hang", "invalid", "environment"])
