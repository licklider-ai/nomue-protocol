...
import { publishReceipt, validatePublic, flattenChecks } from "./public.mjs";
...
assert.equal(flattenChecks(success.output).length, 5);
assert.ok(
  flattenChecks(success.output).every((c) => c.outcome === "pass" && c.reason_codes.length === 0),
);
assert.equal(success.verified_record_base64, baseline.verified_bytes.toString("base64"));
rows.push("five scoped passes separated by role and original bytes");
assert.equal(success.output.checks, undefined);
assert.deepEqual(
  success.output.conformance.checks.map((c) => c.stage),
  ["declaration", "admission"],
);
assert.deepEqual(
  success.output.verification_results.map((c) => c.stage),
...
for (const [field, reason] of [
  ...
  assert.deepEqual(flattenChecks(result.output)[4].reason_codes, [reason]);
  assert.equal(flattenChecks(result.output)[4].outcome, "fail");
...
assert.equal(flattenChecks(error.output)[4].execution, "error");
assert.equal(flattenChecks(error.output)[4].outcome, undefined);
assert.deepEqual(flattenChecks(error.output)[4].reason_codes, ["candidate:holm:worker_failure"]);
...
for (const edit of [
  ...
  (r) => {
    flattenChecks(r)[0].outcome = "fail";
    flattenChecks(r)[0].reason_codes = ["candidate:holm:digest_mismatch"];
  },
  (r) => {
    flattenChecks(r)[4].scope.result_id = "other";
  },
  (r) => {
    flattenChecks(r)[4].reason_codes = ["candidate:holm:context_mismatch"];
  },
]) {
