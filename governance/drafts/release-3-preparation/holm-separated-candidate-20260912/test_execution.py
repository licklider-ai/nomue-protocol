...
def main():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if a.delegation:
            ...
            def checkpoint_timeout():
                r = want(a.delegation, "checkpoint-timeout", "completed_valid")
                assert r["result"]["output"]["reason"] == "processing_timeout", r
                assert "verified_record_base64" not in r["result"], r
                assert "checks" not in r["result"]["output"], r
                return r
            check("real 5-second checkpoint refusal and cleanup", checkpoint_timeout)
            def environment():
