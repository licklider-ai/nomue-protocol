...
function check(d) {
  ...
  for (const id of all)
    assert.match(id, /^https:\/\/nomue\.ai\/id\/[a-z]+\/[a-z]+(?:-[a-z]+)*\/0\.3\.0-candidate\.4$/);
  ...
  const surface = d["report.schema"].properties;
  const checks = [
    surface.verification_results.prefixItems[0],
    surface.verification_results.prefixItems[1],
    ...surface.conformance.properties.checks.prefixItems,
    surface.verification_results.prefixItems[2],
  ];
...
const mutations = [
  ...
  [
    ...
    (d) => {
      d[
        "report.schema"
      ].properties.verification_results.prefixItems[0].allOf[1].properties.check_ref.const =
