...
paths.add(str((here/'probes.mjs').relative_to(root)))
(here/'INPUTS.json').write_text(json.dumps({'base_commit':'4a62f8e1768049560cb0ce8f09ef1676cb42130b','status':'unissued conformance-separated successor; historical candidates unchanged','runtime':[{'path':p,'sha256':hashlib.sha256((root/p).read_bytes()).hexdigest()}for p in sorted(paths)]},indent=2)+'\n')
