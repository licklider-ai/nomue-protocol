...
import {
  ...
  storedDigest,
  projectStoredBytes,
  validateOutput as validatePrivate,
} from "./envelope.mjs";
...
function boundBytes(bytes, report) {
  try {
    const record = parseStrictJson(new TextDecoder("utf-8", { fatal: true }).decode(bytes));
    return (
      record.integrity?.content_digest === report.record_reference.content_digest &&
      record.record_id === report.record_reference.record_id &&
      record.revision_id === report.record_reference.revision_id &&
      storedDigest(projectStoredBytes(bytes)) === report.record_reference.content_digest
    );
  } catch {
    return false;
  }
...
export async function validatePublic(output) {
  if (reportSchema(output)) {
    ...
    r.checks = flattenChecks(r);
    delete r.conformance;
    delete r.verification_results;
    r.kind = IDS.report;
...
export async function publishReceipt(receipt, { now = clock } = {}) {
    ...
    if (!transport || !(await validatePrivate(transport.output))) {
      ...
      if (Object.hasOwn(transport, "verified_record_base64")) {
        ...
        if (typeof b64 !== "string" || b64.length > 4 * Math.ceil(CAPS.recordBytes / 3))
          ...
          if (
            !allPass ||
            b.length > CAPS.recordBytes ||
            b.toString("base64") !== b64 ||
            !boundBytes(b, r)
          )
            result = {
              output: refusal("execution", "internal_error", "completed_invalid_output", now),
  ...
  if (result.output.report_type === "nomue-verification-report") {
    const r = result.output;
    const checks = r.checks;
    r.conformance = {
      schema: { schema_id: IDS.record, execution: "completed", outcome: "pass" },
      checks: [checks[2], checks[3]],
    };
    r.verification_results = [checks[0], checks[1], checks[4]];
    delete r.checks;
  }
...
// Presentation partition only; private evaluation order and gating are unchanged.
// Callers validate the public schema before using this inverse projection.
export function flattenChecks(report) {
  return [
    report.verification_results[0],
    report.verification_results[1],
    report.conformance.checks[0],
    report.conformance.checks[1],
    report.verification_results[2],
  ];
}
