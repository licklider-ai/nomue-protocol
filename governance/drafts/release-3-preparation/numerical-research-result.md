...
**Program disposition: `NARROW`** (Section 17). Every catalogue entry receives a numerical
family and a per-procedure disposition (Sections 6 and 17); no technique is silently
treated as numerically covered.
...
result. All probes ran on the research container (Linux, x86-64) under CPython 3.11.15.
...
Release 2 and Release 3 enclosures in one environment. Probe scripts (A through G, plus the C-2 completion script) are reproduced in full in
...
- **N-02 (`PROOF`, `PROBE` A).** When both `a = d2/2` and `b = d1/2` are positive integers,
  ...
  needed 142,200 denominator bits and 0.65 s).
...
- **N-15 (`PROBE` D, resources).** Exact closed testing over all `2^m - 1` intersections
  took 0.01 s at `m = 10`, 0.82 s at `m = 16`, and 16.0 s at `m = 20` in the probe
  environment; exact Holm at `m = 100,000` took 1.8 s; exact quadratic Hommel at
  `m = 2,000` took 12.3 s; the exact power `(1-p)^m` for the smallest subnormal `p` has
  `1074·m + 1` denominator bits (10.7 million bits and 1.3 s at `m = 10,000`). These are
...
- **N-26 (`PROBE` B, `PROBE` G).** A rigorous enclosure route for `P(Q_{k,ν} ≤ q)` was
  ...
  (347 s per bracket), and at tolerance `2^-56` with 128-bit precision the enclosure radius
  at the root falls to `5.3e-18` in 270 s, below the cell requirement. Cell-level
  certification of Studentized-range quantiles is therefore feasible at roughly five
  minutes per evaluation in this environment (two evaluations per cell), which is a resource
...
- **N-28 (`PROBE` C, `PROBE` C-2).** A rigorous enclosure route (same truncation and
  ...
  cases: balanced `p = 3` one-sided (`ν = 16`, radius `4.0e-11`, 226 s), unbalanced `p = 3`
  two-sided (`ν = 30`, `4.9e-12`, 352 s), balanced `p = 2` two-sided (`ν = 9`, `5.8e-11`,
  106 s), and balanced `p = 8` two-sided (`ν = 171`, `7.3e-12`, 1,938 s); every
  ...
  `0.7857`). Probe C's final statement failed on a scripting error (a malformed
  `zip` expression, recorded verbatim in Appendix B) after all numerical sections had
  completed; Probe C-2 re-ran the remaining boundary checks with the unchanged routes:
...
Numerical dispositions (this report's vocabulary; orthogonal to the semantic labels):
`NUM-FEASIBLE` (at least two independent evidence routes executed and a runtime candidate
pattern identified), `NUM-FEASIBLE-TABLE` (feasible for the rejection set and fixed-level
intervals through an exhaustively certified finite table; the distribution function at
arbitrary arguments remains oracle-only), `NUM-ORACLE-ONLY` (truth reference feasible by
enclosure; no runtime binary64 graph with an error proof exists), `NUM-DEFER` (a
reproducible truth reference, projection boundary, or resource-bounded path cannot be
defended now), `NUM-TRANSFER` (follows the semantic transfer; randomness or rank foundation
not closed), `NUM-NA` (no numerical path needed). "Gate" names the semantic hold that
blocks any Contract regardless of numerical feasibility.

| ID     | Procedure                            | Semantic disposition | Family            | Required quantities                                                                                 | Numerical disposition                                                        | Gate                      |
| ------ | ------------------------------------ | -------------------- | ----------------- | --------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- | ------------------------- |
| OMN-01 | Classical one-way ANOVA F            | `R3-CAND`†           | NF-A              | F statistic algebra; F upper tail at integer `(k-1, N-k)`; F quantile for a fixed level             | `NUM-FEASIBLE`                                                               | SR-A                      |
| OMN-02 | Welch heteroscedastic omnibus        | `R3-CAND`†           | NF-A              | weighted statistic; non-integer denominator df; F tail at non-integer `d2`                          | `NUM-ORACLE-ONLY`                                                            | SR-A                      |
| OMN-03 | James first/second order             | `RES-ONLY`†          | NF-A              | chi-square tails plus series corrections (not probed)                                               | `NUM-DEFER`                                                                  | SR-A                      |
| OMN-04 | Brown-Forsythe modified F            | `RES-ONLY`†          | NF-A              | F tail at non-integer `d2`                                                                          | `NUM-ORACLE-ONLY`                                                            | SR-A                      |
| OMN-05 | Kruskal-Wallis                       | `TRANSFER`           | NF-G              | rank transform, ties, chi-square tail or exact permutation distribution                             | `NUM-TRANSFER`                                                               | —                         |
| OMN-06 | Permutation one-way tests            | `TRANSFER`           | NF-F              | randomness identity, permutation scheme, replicate count                                            | `NUM-TRANSFER`                                                               | —                         |
| PVL-01 | Bonferroni                           | `R3-CAND`            | NF-B (+NF-A)      | `min(1, m p)`; per-member `t` quantile at `α/(2m)` for rectangular intervals                        | `NUM-FEASIBLE`                                                               | SR-B (attribution only)   |
| PVL-02 | Šidák                                | `R3-CAND`†           | NF-B              | exact `(1-p)^m` decision; correctly rounded `1-(1-p)^m` report                                      | `NUM-FEASIBLE`                                                               | SR-B                      |
| PVL-03 | Holm step-down                       | `R3-CAND`            | NF-B              | stable sort, running maximum, `α/(m-r)` thresholds                                                  | `NUM-FEASIBLE`                                                               | —                         |
| PVL-04 | Holm product-form                    | `R3-CAND`            | NF-B              | exact `(1-p)^{m-r} ≥ 1-α` decisions                                                                 | `NUM-FEASIBLE`                                                               | —                         |
| PVL-05 | Weighted Holm                        | `RES-ONLY`           | NF-B              | rational weights; weighted thresholds                                                               | `NUM-FEASIBLE`                                                               | —                         |
| PVL-06 | Simes global test                    | `RES-ONLY`†          | NF-B              | `min_i (m/i) p_(i)`                                                                                 | `NUM-FEASIBLE`                                                               | SR-C                      |
| PVL-07 | Hochberg step-up                     | `R3-CAND`†           | NF-B              | sort, running minimum from the top                                                                  | `NUM-FEASIBLE`                                                               | SR-C                      |
| PVL-08 | Hommel                               | `RES-ONLY`†          | NF-B/NF-E         | quadratic-time closed-Simes algorithm; exponential definition for small-`m` cross-checks            | `NUM-FEASIBLE` (with ceiling)                                                | SR-C                      |
| PVL-09 | Shaffer logically restricted         | `RES-ONLY`†          | NF-B              | per-step maximal number of true hypotheses under logical constraints (integer sequence); thresholds | `NUM-FEASIBLE` (integer sequence must be versioned)                          | SR-C                      |
| PVL-10 | Rom; Holland-Copenhaver              | `RES-ONLY`†          | NF-B              | Rom constants (polynomial recursion in `α`); product-form step-down                                 | `NUM-FEASIBLE` (constants exact-rational only if the recursion is versioned) | SR-C                      |
| CLS-01 | Closed testing principle             | `R3-CAND`†           | NF-E              | all `2^m - 1` intersection tests over a local-test family                                           | `NUM-FEASIBLE` (with ceiling)                                                | SR-D                      |
| CLS-02 | Fixed-sequence                       | `RES-ONLY`†          | NF-B              | ordered comparison at full `α`                                                                      | `NUM-FEASIBLE`                                                               | SR-D                      |
| CLS-03 | Fallback                             | `RES-ONLY`†          | NF-B              | rational `α`-propagation bookkeeping                                                                | `NUM-FEASIBLE`                                                               | SR-D                      |
| CLS-04 | Serial gatekeeping                   | `RES-ONLY`†          | NF-B/NF-E         | family-level tests (NF-B or NF-E) in sequence                                                       | `NUM-FEASIBLE` (with ceiling)                                                | SR-D                      |
| CLS-05 | Parallel gatekeeping                 | `RES-ONLY`†          | NF-B/NF-E         | same                                                                                                | `NUM-FEASIBLE` (with ceiling)                                                | SR-D                      |
| CLS-06 | Graphical weighted Bonferroni        | `RES-ONLY`†          | NF-B              | rational weight/transition matrices; iterative `α` reallocation                                     | `NUM-FEASIBLE`                                                               | SR-D                      |
| APR-01 | Balanced Studentized-range intervals | `R3-CAND`            | NF-C              | `q_{α;k,ν}` at integer `ν`; `S/sqrt(n)`; exact statistic comparison; adjusted p needs `P(Q ≥ q)`    | `NUM-FEASIBLE-TABLE`                                                         | — (SR-E attribution only) |
| APR-02 | Tukey-Kramer intervals               | `R3-CAND`            | NF-C              | same constant; per-pair `sqrt((1/n_i+1/n_j)/2)` (exact rational under the square)                   | `NUM-FEASIBLE-TABLE`                                                         | —                         |
| APR-03 | Kramer multiple-range test           | `RES-ONLY`           | NF-C              | `q` at varying span `2..k`                                                                          | `NUM-FEASIBLE-TABLE`                                                         | —                         |
| APR-04 | Spjøtvoll-Stoline extended T         | `RES-ONLY`           | NF-C'             | augmented Studentized range quantile; `max(a_i, a_j)`                                               | `NUM-DEFER`                                                                  | —                         |
| APR-05 | Hochberg GT2                         | `RES-ONLY`†          | NF-C'             | Studentized maximum modulus quantile                                                                | `NUM-DEFER`                                                                  | SR-F                      |
| APR-06 | Genizi-Hochberg                      | `RES-ONLY`†          | NF-C'             | special tables (uninspected)                                                                        | `NUM-DEFER`                                                                  | SR-F                      |
| APR-07 | Gabriel                              | `REJECT`             | NF-0              | —                                                                                                   | `NUM-NA`                                                                     | —                         |
| APR-08 | Harmonic-mean substitution           | `REJECT`             | NF-0              | —                                                                                                   | `NUM-NA`                                                                     | —                         |
| APR-09 | Scheffé all-contrasts                | `R3-CAND`†           | NF-A              | F quantile at `(k-1, ν)`; exact `contrast^2/var ≤ (k-1)F` surface                                   | `NUM-FEASIBLE`                                                               | SR-G                      |
| APR-10 | Newman-Keuls                         | `RES-ONLY`†          | NF-C              | `q` at spans `2..k`                                                                                 | `NUM-FEASIBLE-TABLE`                                                         | SR-H                      |
| APR-11 | Duncan                               | `RES-ONLY`†          | NF-C              | `q` at span-dependent levels `1-(1-α)^{s-1}`                                                        | `NUM-ORACLE-ONLY`                                                            | SR-H                      |
| APR-12 | REGWQ                                | `RES-ONLY`†          | NF-C              | `q` at span-dependent levels `1-(1-α)^{s/k}`                                                        | `NUM-ORACLE-ONLY`                                                            | SR-H                      |
| APR-13 | Fisher protected LSD                 | `RES-ONLY`†          | NF-A              | F tail then `t` quantile                                                                            | `NUM-FEASIBLE`                                                               | SR-H                      |
| APR-14 | Hayter modified LSD                  | `RES-ONLY`†          | NF-C              | `q_{α;k-1,ν}/sqrt 2`                                                                                | `NUM-FEASIBLE-TABLE`                                                         | SR-H                      |
| HET-01 | Games-Howell                         | `R3-CAND`†           | NF-C (+NF-A)      | per-pair Welch-Satterthwaite `ν_ij` (non-integer); `q_{α;k,ν_ij}`                                   | `NUM-ORACLE-ONLY`                                                            | SR-I                      |
| HET-02 | Tamhane T2                           | `RES-ONLY`†          | NF-A/NF-B         | per-pair `t` quantile at Šidák-adjusted level with non-integer `ν_ij`                               | `NUM-ORACLE-ONLY`                                                            | SR-I                      |
| HET-03 | Dunnett T3 and C                     | `RES-ONLY`†          | NF-C'/NF-C        | Studentized maximum modulus (T3) or `q` (C) at non-integer `ν_ij`                                   | `NUM-DEFER`                                                                  | SR-I                      |
| MTO-01 | Dunnett many-to-one intervals        | `R3-CAND`            | NF-D              | equicoordinate constant at `(p, ν, λ_1..λ_p)`; one- and two-sided                                   | `NUM-FEASIBLE-TABLE` (balanced grid) / `NUM-ORACLE-ONLY` (arbitrary sizes)   | —                         |
| MTO-02 | Step-down Dunnett                    | `RES-ONLY`†          | NF-D              | constants at shrinking subset sizes                                                                 | `NUM-ORACLE-ONLY`                                                            | SR-J                      |
| MTO-03 | Step-up Dunnett-Tamhane              | `RES-ONLY`†          | NF-D              | step-up constants (own tables)                                                                      | `NUM-DEFER`                                                                  | SR-J                      |
| MCB-01 | Hsu MCB                              | `RES-ONLY`†          | NF-D              | one-sided Dunnett-type constants with constrained intervals                                         | `NUM-ORACLE-ONLY`                                                            | SR-J                      |
| FDR-01 | Benjamini-Hochberg                   | `R3-CAND`            | NF-B              | sort, `(i/m) q` thresholds, running minimum                                                         | `NUM-FEASIBLE`                                                               | — (dependence scope SR-K) |
| FDR-02 | Benjamini-Yekutieli                  | `R3-CAND`†           | NF-B              | exact harmonic constant; thresholds                                                                 | `NUM-FEASIBLE`                                                               | SR-K                      |
| FDR-03 | Adaptive / two-stage BH              | `RES-ONLY`†          | NF-B              | `m_0` estimator (integer count from a first pass); re-thresholding                                  | `NUM-FEASIBLE` (once the estimator is versioned)                             | SR-K                      |
| FDR-04 | Storey q-value                       | `TRANSFER`           | NF-B (+smoothing) | `π_0` estimation with a tuning parameter or spline smoothing                                        | `NUM-TRANSFER`                                                               | SR-K                      |
| RSM-01 | Westfall-Young maxT/minP             | `TRANSFER`           | NF-F              | randomness identity, permutation scheme, `B`, max statistic, empirical tail                         | `NUM-TRANSFER`                                                               | —                         |
| RSM-02 | Permutation pairwise/step-down       | `TRANSFER`           | NF-F              | same                                                                                                | `NUM-TRANSFER`                                                               | —                         |

Counts (49 entries): `NUM-FEASIBLE` 22, `NUM-FEASIBLE-TABLE` 6 (APR-01, APR-02, APR-03,
APR-10, APR-14, MTO-01 balanced), `NUM-ORACLE-ONLY` 9 (OMN-02, OMN-04, APR-11, APR-12,
HET-01, HET-02, MTO-02, MCB-01, and MTO-01 at arbitrary sizes counted with its table
entry), `NUM-DEFER` 6 (OMN-03, APR-04, APR-05, APR-06, HET-03, MTO-03), `NUM-TRANSFER` 5
(OMN-05, OMN-06, FDR-04, RSM-01, RSM-02), `NUM-NA` 2 (APR-07, APR-08). The two guidance
...
`NUM-FEASIBLE`; APR-01 and APR-02 are `NUM-FEASIBLE-TABLE`; MTO-01 is
`NUM-FEASIBLE-TABLE` for a balanced or finite size-configuration grid and
`NUM-ORACLE-ONLY` for arbitrary size configurations.
...
`NUM-TRANSFER`, consistent with the semantic transfer; no deterministic approximation is
...
| NB-06   | Augmented range and maximum modulus (NF-C') were not executed; APR-04/05/06 and HET-03 are `NUM-DEFER`.                                                                                           | Derivation of the inner functional, execution of the two-route probe, and independent review.                                                                  |
...
| NB-10   | The randomness contract for NF-F has no adopted generator, mapping, or replicate-count semantics.                                                                                                 | Closure of the seeded-stochastic foundation line; until then `NUM-TRANSFER`.                                                                                   |

### 16.2 Semantic holds that also block numerical closure

SR-A through SR-L remain as recorded in the semantic result; this report neither closes
nor re-adjudicates them. Numerical work on a held entry may proceed as oracle evidence but
...
### 17.1 Program disposition: `NARROW`

- Why not `NUMERIC_PROGRAM_READY`: that label requires every Release 3 implementation
  candidate to have either a feasible evidence path or an explicit blocking hold. The
  evidence path exists for the algebraic families and for the F/t family at integer
  degrees of freedom, and a table path exists for fixed-level range and many-to-one
  constants; but the seven semantically unblocked candidates include APR-01, APR-02, and
  MTO-01, whose adjusted p-values and arbitrary-size constants are oracle-only (NB-01,
  NB-07), and the primary algorithmic and standards literature for the nested-integral
  families could not be inspected (NB-03). Treating those as "covered" would be exactly the
  silent numerical coverage the label forbids.
- Why not `DEFER` or `NO_GO`: a reproducible truth reference, a projection boundary, and a
  resource-bounded path were defended by two independent routes for NF-A and NF-B and by
  two routes plus exact identities for NF-C and NF-D oracles (Section 4), including a
  demonstrated cell-level enclosure precision for a Studentized-range quantile (N-26). The Release 2
  patterns transfer in form to these families (Section 14.1). Bounded work can start now on
  the D2 (generic p-value) lane with an exact decision layer, and on fixed-level table
  instruments for D3/D4, while the oracle-only items stay explicitly held.
- What `NARROW` commits to: nothing. No path, tolerance, table, ceiling, platform, or
  stochastic contract is selected; the per-procedure dispositions below are numerical
  feasibility classifications for the RFC ledger.

### 17.2 Per-procedure dispositions

The numerical disposition of every catalogue entry is given in the Section 6 matrix
(column "Numerical disposition"). Summary by class:

- `NUM-FEASIBLE` (22): OMN-01, PVL-01, PVL-02, PVL-03, PVL-04, PVL-05, PVL-06, PVL-07,
  PVL-08, PVL-09, PVL-10, CLS-01, CLS-02, CLS-03, CLS-04, CLS-05, CLS-06, APR-09, APR-13,
  FDR-01, FDR-02, FDR-03.
- `NUM-FEASIBLE-TABLE` (6): APR-01, APR-02, APR-03, APR-10, APR-14, MTO-01 (balanced or
  finite size grid).
- `NUM-ORACLE-ONLY` (9): OMN-02, OMN-04, APR-11, APR-12, HET-01, HET-02, MTO-02, MCB-01,
  MTO-01 (arbitrary sizes).
- `NUM-DEFER` (6): OMN-03, APR-04, APR-05, APR-06, HET-03, MTO-03.
- `NUM-TRANSFER` (5): OMN-05, OMN-06, FDR-04, RSM-01, RSM-02.
- `NUM-NA` (2): APR-07, APR-08.

A `NUM-FEASIBLE` or `NUM-FEASIBLE-TABLE` label never overrides a semantic hold; the "Gate"
column of Section 6 governs Contract eligibility.
...
Each script is reproduced exactly; its SHA-256 is the digest of the file bytes as run. Reproduction: CPython 3.11.15 with the Section 2.3 packages; run each script from an empty working directory.
...
| `probe_g_range_bracket.py`        | `04ddc7a51143e5dc8a0e24f03ef7dab1ab6631767cdf49ace42afc1d9f149be6` |
| `probe_c_dunnett.py`              | `66bd8cd235fc495e7aaf3614c5880007baf98b631c363febad3acfc499f643f0` |
| `probe_c2_dunnett_boundaries.py`  | `ec7fa3f70e2a18b7adba67c25f1aa57f56385d35fc34405e7b888f13251d14ea` |
...
from fractions import Fraction
sys.path.insert(0, '/tmp/claude-0/-home-user-nomue-protocol/d5792af6-99ef-5a5f-ac41-1608368f0b54/scratchpad/probes')
...
if __name__ == '__main__':
    ...
    print('d monotone:', all(a < b for a, b in zip(*[[dunnett_mp([5,5], 5, 12, mpf(d)) for d in [0.5, 1, 2, 3, 5]]]*1)))
...
### A.5 Probe C-2 — many-to-one boundary completion (`probe_c2_dunnett_boundaries.py`)

```python
"""Probe C-2: completes the boundary section of Probe C after its last statement failed
on a scripting error (a malformed `zip(*...)` expression, not a numerical failure).
Reuses Probe C's routes unchanged."""
import sys
sys.path.insert(0, '/tmp/claude-0/-home-user-nomue-protocol/d5792af6-99ef-5a5f-ac41-1608368f0b54/scratchpad/probes')
...
Outputs are reproduced as captured on 2026-09-04; wall-clock timings are environment observations only. Long numeric lines are unaltered.

### B.1 Probe A — F and t tails output (`probe_a.out`, SHA-256 `094b0cb06485615d4fd8929939e4752d7299a688aedd5acba04b16a46709324e`)
...
versions mpmath 1.4.1 python-flint 0.9.0 scipy 1.17.1 python 3.11.15
```

### B.2 Probe B — Studentized range output (`probe_b.out`, SHA-256 `a89ba5a1f7f1eef6ac4752952197c935b114650ba49a5f6c167818ba2b697c2d`)
...
### B.3 Probe G — Studentized range bracket at declared resolution output (`probe_g.out`, SHA-256 `784f2851baacdc0aee9456b54721a12120a2e6f31c6ad260116d30a2b5a8e36d`)

```text
=== declared-resolution bracket h = 1e-12 around 3.876776750013182
h=1e-12: P(q0-h)-0.95 = [-6e-14 +/- 4.96e-15] negative; P(q0+h)-0.95 = [6e-14 +/- 4.89e-15] positive; certified=True (347s)
h=1e-13: P(q0-h)-0.95 = [+/- 8.52e-15] negative; P(q0+h)-0.95 = [+/- 8.59e-15] positive; certified=True (351s)
=== enclosure radius versus requested tolerance at q0 (binary64-cell bracket needs radius below ~4e-17)
tol=2^-40 prec=96: [0.950000000000 +/- 9.92e-14] radius=[9.91e-14 +/- 8.67e-18] (172s)
tol=2^-48 prec=96: [0.95000000000000 +/- 2.80e-15] radius=[2.76e-15 +/- 2.77e-18] (170s)
tol=2^-56 prec=128: [0.95000000000000001 +/- 6.16e-18] radius=[5.32e-18 +/- 1.26e-22] (270s)
local density at q0 ~ 0.0578404 ; CDF change across one binary64 ULP ( 4.440892098500626e-16 ) ~ 2.569e-17
```

### B.4 Probe C — many-to-one multivariate t output (`probe_c.out`, SHA-256 `e2a3fd5750a2c9c8f73bda0c6455e83f1afd802ca95745a034edfe698b9367a1`)
...
ns=[5, 9] n0=7 nu=18 d=2.0 two_sided=False: C1=0.9458290493023317 (16.3s) C2=0.9458290493023317 (208.3s) rel=0.0
ns=[5, 9] n0=7 nu=18 d=2.2 two_sided=True: C1=0.9258209328927715 (33.9s) C2=0.9258209328927715 (88.7s) rel=0.0
ns=[3, 30] n0=4 nu=34 d=2.1 two_sided=False: C1=0.9625224804271421 (23.7s) C2=0.9625224804271421 (274.9s) rel=1.13e-19
ns=[10, 10] n0=10 nu=27 d=1.92 two_sided=False: C1=0.9418526167689369 (16.8s) C2=0.9418526167689369 (233.0s) rel=0.0
=== C1 vs C3 (Arb rigorous)
ns=[5, 5, 5] n0=5 nu=16 d=2.3 two_sided=False: C1=0.9563104423686814 (23.0s) C3=[0.9563104424 +/- 3.98e-11] (225.5s) C1_inside_C3=True
ns=[5, 9, 13] n0=7 nu=30 d=2.6 two_sided=True: C1=0.9631009000195884 (55.1s) C3=[0.96310090002 +/- 4.93e-12] (351.5s) C1_inside_C3=True
ns=[4, 4] n0=4 nu=9 d=2.5 two_sided=True: C1=0.9400080640433291 (30.9s) C3=[0.9400080640 +/- 5.79e-11] (105.5s) C1_inside_C3=True
ns=[20, 20, 20, 20, 20, 20, 20, 20] n0=20 nu=171 d=2.6 two_sided=True: C1=0.9385230853449858 (160.0s) C3=[0.93852308534 +/- 7.34e-12] (1937.9s) C1_inside_C3=True
...
Traceback (most recent call last):
  File "/tmp/claude-0/-home-user-nomue-protocol/d5792af6-99ef-5a5f-ac41-1608368f0b54/scratchpad/probes/probe_c_dunnett.py", line 160, in <module>
    print('d monotone:', all(a < b for a, b in zip(*[[dunnett_mp([5,5], 5, 12, mpf(d)) for d in [0.5, 1, 2, 3, 5]]]*1)))
                         ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  File "/tmp/claude-0/-home-user-nomue-protocol/d5792af6-99ef-5a5f-ac41-1608368f0b54/scratchpad/probes/probe_c_dunnett.py", line 160, in <genexpr>
    print('d monotone:', all(a < b for a, b in zip(*[[dunnett_mp([5,5], 5, 12, mpf(d)) for d in [0.5, 1, 2, 3, 5]]]*1)))
                                       ^^^^
ValueError: not enough values to unpack (expected 2, got 1)
```

### B.5 Probe C-2 — many-to-one boundary completion output (`probe_c2.out`, SHA-256 `5742ab24c12bb2d03f99cd07560d6487b0fa385475fe858484c90a45bc39820a`)
...
### B.6 Probe D — adjusted-p arithmetic output (`probe_d.out`, SHA-256 `3907f1ba0f1b07221135f48f5b6f33b2fdb2a15dfc4bf9c087d188911675f4fd`)
...
resource: exact (1-p)^m denominator bits and time
  ...
  m=1000: denominator bits 1074001 time 0.039s
  m=10000: denominator bits 10740001 time 1.300s
...
closed testing m=16: subsets 65535 time 0.82s
closed testing m=20: subsets 1048575 time 16.04s
exact Holm m=100000: 1.78s
exact Hommel O(m^2) m=2000: 12.34s
...
### B.7 Probe E — stochastic replay and intervals output (`probe_e.out`, SHA-256 `810e030248057f7a25b0d4d6d2da960a3b390a265a8ae40aec2b364b6db1aa1a`)

```text
=== Part 1: stochastic candidate replay / cross-seed
seed=1 B=2000: p=104/2001 (0.05197) count=103 (0.3s); replay identical: True
...
exact enumeration: p = 1830/34650 = 0.05281 (3.8s); permutation estimates above scatter around it
...
### B.8 Probe F — quantile certificates and resources output (`probe_f.out`, SHA-256 `695f90873ba4af2b22bccd07693231d9231b6415b31419bbde717060a767c682`)
...
F(50,5000) f=1.2: exact tail denominator bits 142200 time 0.652s value 0.15876795897766818
versions mpmath 1.4.1 python-flint 0.9.0
```
