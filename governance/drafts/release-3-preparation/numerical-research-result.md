...
the repair scope and independence boundary. That repair preserved the mathematical derivations.
It regenerated seven probe transcripts and retained Probe
B's complete original transcript with its corrected digest; a
...
The optional repository-wide audit enhancement was outside that one-path repair.

**Independent reproduction and corrective proposal (2026-09-09).** The
[reproduction review](../../../review-inputs/r3-numerical-reproduction/REVIEW-RESULT.md)
assesses PR #174 at `9f39eafd4b0a676e6615956b5a7899f195fc0358`. Its reviewer did not
author the investigation or September 4 repair. This successor regenerates the output
evidence, discloses B/C parallel replay, and narrows certification language to the
boundaries actually evaluated. The review records prior involvement, software assistance,
execution provenance, and the limit on self-review of these new corrections. No missing
primary-source requirement or numerical hold is waived.

The September 4 repair also read the then-merged semantic source-acquisition record at main
...
numerical observation. This is a historical snapshot, not a replacement for later accepted
source work. The separate approved SR-J acceptance and its source-requirement changes are
not reopened here.
...
remaining duplicate resource-intensive portion. The September 9 reproduction replaces
Appendix B with newly captured output on CPython 3.12.14, Linux x86_64, with the same
four active package versions and the CPython 3.12 wheel identities below. A, G, C-2, D,
E, and F execute as complete unmodified scripts. B and C are replayed by independent
sections/cases using the disclosed driver; Appendix B concatenates their raw task stdout
in script order. It is not represented as one serial-process transcript.
...
`gmpy2` was not installed in the repair or September 9 reproduction environment.
The reproduction directly confirmed `mpmath.libmp.BACKEND = python`. An installed
gmpy2 can be selected automatically by mpmath, so absence of a direct probe import
does not prove that it was unused in the original investigation.
...
and reproduced under the capture convention in Appendix B. Resource timings in Section 4
...
- **N-22 (`PROBE` F).** For six `(d1, d2)` pairs including a non-integer `d2 = 7.3`, the
  ...
  (N-02) independently confirmed the bracket. These are signs at neighbouring binary64
  values, not at exact rounding-cell midpoints. They demonstrate a quantile bracket;
  certification of a correctly rounded table cell additionally requires the midpoint
  containment checks specified in Section 9. No such table-cell certification is
  claimed from these endpoint evaluations alone.
...
- **N-25 (`PROOF`, `PROBE` B).** Let `Z_1..Z_k` be iid `N(0,1)`, `R = max Z_i - min Z_i`,
  ...
  (condition on the maximum being at `z`: the remaining `k-1` values lie in `[z-w, z]`,
  and the factor `k` counts which variable is the maximum). Two exact anchors follow:
...
- **N-26 (`PROBE` B, `PROBE` G).** A rigorous enclosure route for `P(Q_{k,ν} ≤ q)` was
  ...
  neighbour). The root is therefore bracketed between the lower neighbour and
  `3.151064183329408`. This script did not evaluate exact rounding-cell midpoints;
  that candidate is not certified as correctly rounded by these signs alone.
...
The commissioned fixed semantic result records SR-A through SR-L. At the historical
source-acquisition intake `0eb388e1…`, SR-L was `CLOSED`; SR-A through SR-K and the
RSM-01/RSM-02 source work were incomplete. This dated statement does not supersede
later accepted semantic source records. This report neither closes nor re-adjudicates
...
## 19. Reproduction and repair verification

The [independent reproduction review](../../../review-inputs/r3-numerical-reproduction/REVIEW-RESULT.md)
assesses the repaired PR #174 head and records the corrections proposed here:

1. The program result remains `INPUT_INCOMPLETE` without a numerical disposition.
   Preliminary labels authorize no implementation, selection, or public discussion.
2. All eight Appendix A scripts retain their exact reviewed bytes and digests. Every
   Appendix B output digest binds newly captured bytes, including the final newline.
3. A, G, C-2, D, E, and F completed as unmodified serial scripts. B and C were replayed
   in 24 and 17 independent tasks respectively, with every task exiting 0 and no
   stderr. Their full ordered task outputs are preserved, with no case omitted.
4. The matrix contains 49 catalogue IDs and 50 scope assignments. MTO-01 retains its
   separate balanced/fixed-grid and arbitrary-size assessments.
5. G and C-2 import sibling modules. Probe C's actual trailing output space is
...
make the capture identities and the finite case coverage checkable. The review states
the independence limit for its accompanying corrections; this is a proposed informative
intake, not an independent approval of this newly authored successor.

## Appendix A. Probe scripts (verbatim) and digests

Each script is preserved exactly from the reviewed head, with the digest of those
source bytes. These are the complete serial reproduction inputs. The September 9
captures execute six scripts directly and replay the B/C sections and cases through
the separately preserved driver; their source files themselves are unchanged. Place
the scripts together in an empty neutral directory. The reproduction review gives
the extraction, parallel replay, and evidence-verification commands.
...
## Appendix B. Newly captured probe outputs

These outputs were captured on 2026-09-09 in the environment recorded in Section 2.3.
A, G, C-2, D, E, and F are verbatim complete serial-process stdout. B and C concatenate
verbatim independent-task stdout in original section and case order, as declared in
Section 19; they are not one serial-process transcript. No old output is substituted
for an unexecuted case. All stderr streams are empty.

Every digest identifies the exact fenced bytes, including the final newline and any
trailing space. Probe C's fence has a local formatting exemption solely to preserve
that byte. Wall-clock values belong to these executions and are not resource bounds.
The dated historical timings in Section 4 remain distinguishable from this new evidence.

### B.1 Probe A — F and t tails output (`probe_a.out`, SHA-256 `8bf03d5a7093396fde0c12a0819f0ff2a1b5c5321e115876f95f6bb090e5cc00`)
...
versions mpmath 1.4.1 python-flint 0.9.0 scipy 1.17.1 python 3.12.14
```

### B.2 Probe B — Studentized range output (`probe_b.out`, SHA-256 `5abc6d7b7afa2a53a2a2dcda3f2097fe197ce41a395db9063a342432e68cb521`)

```text
=== B3 identity: k=2, P(Q<=q) = P(|T_nu| <= q/sqrt2) = 1 - I_{nu/(nu+t^2)}(nu/2, 1/2)
nu=5 q=2.0 identity=0.78356277073031435187 B1=0.78356277073031435187 rel=0.0 (28.1s)
nu=10 q=3.0 identity=0.94010967557444058025 B1=0.94010967557444058025 rel=0.0 (19.2s)
nu=30 q=4.0 identity=0.99174266408536862777 B1=0.99174266408536862777 rel=0.0 (22.2s)
nu=1 q=1.0 identity=0.39182655203060727017 B1=0.39182655203060727017 rel=0.0 (17.5s)
nu=2 q=8.0 identity=0.97014250014533189408 B1=0.97014250014533189408 rel=8.73e-22 (30.2s)
nu=200 q=2.7718 identity=0.94861028104137728435 B1=0.94861028104137728435 rel=8.93e-22 (38.2s)
...
k=3 nu=10 q=3.877 B1=0.950012911246745594 (20.1s)  B2=[0.950012911247 +/- 3.54e-13] (161.4s) B1_inside_B2=True
k=3 nu=10 q=3.0 B1=0.865016584810436003 (17.3s)  B2=[0.8650165848104 +/- 4.66e-14] (96.6s) B1_inside_B2=True
k=4 nu=20 q=3.958 B1=0.949978788864849565 (19.2s)  B2=[0.94997878886 +/- 8.25e-12] (163.9s) B1_inside_B2=True
k=5 nu=5 q=5.0 B1=0.921074516170004857 (18.6s)  B2=[0.9210745161700 +/- 8.61e-14] (204.3s) B1_inside_B2=True
k=3 nu=2 q=8.0 B1=0.946008999723080908 (17.7s)  B2=[0.94600899972 +/- 4.63e-12] (460.2s) B1_inside_B2=True
k=3 nu=1 q=20.0 B1=0.932630484775861867 (15.6s)  B2=[0.932631 +/- 5.16e-7] (481.2s) B1_inside_B2=True
k=6 nu=60 q=4.16 B1=0.949717156431638243 (14.9s)  B2=[0.94971715643164 +/- 7.01e-15] (192.6s) B1_inside_B2=True
k=3 nu=73/10 q=4.0 B1=0.943499499875563616 (18.9s)  B2=[0.943499499876 +/- 9.06e-13] (1.9s) B1_inside_B2=True
k=20 nu=30 q=5.0 B1=0.894735194056411775 (17.4s)  B2=[0.89473519406 +/- 5.05e-12] (492.9s) B1_inside_B2=True
k=3 nu=10 q=0.5 B1=0.0661358278495920051 (17.7s)  B2=[0.0661358278496 +/- 4.91e-14] (99.7s) B1_inside_B2=True
k=3 nu=10 q=12.0 B1=0.99998102429034977 (31.9s)  B2=[0.999981024290 +/- 4.12e-13] (345.9s) B1_inside_B2=True
...
k=3 nu=10: q=3.876776750013181841516 binary64 nearest 3.876776750013182 (400f03a387599928) (676.3s)
   ...
   (334.9s)
k=4 nu=20: q=3.958293560945356786097 binary64 nearest 3.9582935609453567 (400faa95d081d24f) (864.2s)
   ...
   (351.4s)
k=3 nu=5: q=4.601726054362567298084 binary64 nearest 4.601726054362567 (4012682adff28daa) (499.8s)
   ...
   (400.0s)
k=2 nu=10: q=3.151064183329407960758 binary64 nearest 3.151064183329408 (400935612377f95b) (472.5s)
...
### B.3 Probe G — Studentized range bracket at declared resolution output (`probe_g.out`, SHA-256 `afaaa7a84acd864a1da40949f420dce1d0c06b5c3e7b371ea97448106db64d6b`)

```text
=== declared-resolution bracket h = 1e-12 around 3.876776750013182
h=1e-12: P(q0-h)-0.95 = [-6e-14 +/- 4.96e-15] negative; P(q0+h)-0.95 = [6e-14 +/- 4.89e-15] positive; certified=True (327s)
h=1e-13: P(q0-h)-0.95 = [+/- 8.52e-15] negative; P(q0+h)-0.95 = [+/- 8.59e-15] positive; certified=True (338s)
=== enclosure radius versus requested tolerance at q0 (binary64-cell bracket needs radius below ~4e-17)
tol=2^-40 prec=96: [0.950000000000 +/- 9.92e-14] radius=[9.91e-14 +/- 8.67e-18] (164s)
tol=2^-48 prec=96: [0.95000000000000 +/- 2.80e-15] radius=[2.76e-15 +/- 2.77e-18] (165s)
tol=2^-56 prec=128: [0.95000000000000001 +/- 6.16e-18] radius=[5.32e-18 +/- 1.26e-22] (259s)
local density at q0 ~ 0.0578404 ; CDF change across one binary64 ULP ( 4.440892098500626e-16 ) ~ 2.569e-17
```

### B.4 Probe C — many-to-one multivariate t output (`probe_c.out`, SHA-256 `ff7f3c578d1d121c8c350800ea4d3a92fd983eae445db37d827a5eda5339ac66`)

<!-- Raw task stdout includes one trailing space; preserve its exact hash. -->
...
ns=[5, 9] n0=7 nu=18 d=2.0 two_sided=False: C1=0.9458290493023317 (15.8s) C2=0.9458290493023317 (234.6s) rel=0.0
ns=[5, 9] n0=7 nu=18 d=2.2 two_sided=True: C1=0.9258209328927715 (36.2s) C2=0.9258209328927715 (99.1s) rel=0.0
ns=[3, 30] n0=4 nu=34 d=2.1 two_sided=False: C1=0.9625224804271421 (25.9s) C2=0.9625224804271421 (299.1s) rel=1.13e-19
ns=[10, 10] n0=10 nu=27 d=1.92 two_sided=False: C1=0.9418526167689369 (17.1s) C2=0.9418526167689369 (246.5s) rel=0.0
=== C1 vs C3 (Arb rigorous) 
ns=[5, 5, 5] n0=5 nu=16 d=2.3 two_sided=False: C1=0.9563104423686814 (23.0s) C3=[0.9563104424 +/- 3.98e-11] (225.3s) C1_inside_C3=True
ns=[5, 9, 13] n0=7 nu=30 d=2.6 two_sided=True: C1=0.9631009000195884 (63.2s) C3=[0.96310090002 +/- 4.93e-12] (328.4s) C1_inside_C3=True
ns=[4, 4] n0=4 nu=9 d=2.5 two_sided=True: C1=0.9400080640433291 (33.7s) C3=[0.9400080640 +/- 5.79e-11] (101.0s) C1_inside_C3=True
ns=[20, 20, 20, 20, 20, 20, 20, 20] n0=20 nu=171 d=2.6 two_sided=True: C1=0.9385230853449858 (156.1s) C3=[0.93852308534 +/- 7.34e-12] (1478.0s) C1_inside_C3=True
...
### B.6 Probe D — adjusted-p arithmetic output (`probe_d.out`, SHA-256 `96ded693141121376c0b3a39276da5cbbf2fb73badfdd60f555a326761075daa`)
...
resource: exact (1-p)^m denominator bits and time
  ...
  m=1000: denominator bits 1074001 time 0.044s
  m=10000: denominator bits 10740001 time 1.267s
...
closed testing m=16: subsets 65535 time 0.86s
closed testing m=20: subsets 1048575 time 13.64s
exact Holm m=100000: 1.68s
exact Hommel O(m^2) m=2000: 11.71s
...
### B.7 Probe E — stochastic replay and intervals output (`probe_e.out`, SHA-256 `f0d7bdabe27bb6b58423c96cafce45ba0fb7da6c70e0ace4f7bdb979e25bcc1e`)
...
exact enumeration: p = 1830/34650 = 0.05281 (2.5s); permutation estimates above scatter around it
...
### B.8 Probe F — quantile certificates and resources output (`probe_f.out`, SHA-256 `75b50a38699922e64eb4b412131c3036088c7d9fe6095d4678fb2961d0e2678d`)
...
F(50,5000) f=1.2: exact tail denominator bits 142200 time 0.866s value 0.15876795897766818
versions mpmath 1.4.1 python-flint 0.9.0
```
