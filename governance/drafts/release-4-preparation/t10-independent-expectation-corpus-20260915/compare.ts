...
import { evaluate, validateFinal } from "../t09-check-report-lifecycle-20260915/report.js";
...
execFileSync("python", ["-B", resolve(here, "check_corpus.py")], { cwd: root });
const manifest = readFileSync(resolve(here, "EXPECTATIONS-FREEZE.json"));
const freeze = createHash("sha256").update(manifest).digest("hex");
const fixtures = readFileSync(resolve(here, "corpus/fixtures.jsonl"), "utf8")
  .trim()
  .split("\n")
  .map((s) => JSON.parse(s));
const child = spawn(
